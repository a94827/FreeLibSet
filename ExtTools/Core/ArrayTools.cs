...
{
  /// <summary>
  /// Шаблонный класс, содержащий статическую ссылку на массив нулевой длины
  /// </summary>
  ...
  public static class ArrayTools
  {
    ...
    /// </summary>
    /// <typeparam name="T">Тип данных</typeparam>
    /// <param name="source">Типизированный перечислитель</param>
    /// <returns>Созданный массив</returns>
    ...
    /// <summary>
    /// Создание массива произвольного типа из нетипизированного перечислителя <see cref="IEnumerable"/>.
    /// Если перечислитель возвращает значения, имеющие тип, отличный от требуемого, то выполняется преобразование с помощью <see cref="StdConvert.ChangeType(object, Type)"/>.
    /// </summary>
    /// <typeparam name="T">Тип данных, которые реально использует перечислитель <paramref name="source"/></typeparam>
    /// <param name="source">Нетипизированный перечислитель</param>
    /// <returns>Созданный массив</returns>
    public static T[] CreateArray<T>(IEnumerable source)
    {
      List<T> lst = null;
      ...
      if (source2 == null)
      {
        foreach (object item in source)
        {
          if (lst == null)
            lst = new List<T>();
          lst.Add((T)StdConvert.ChangeType(item, typeof(T)));
        }
      }
      else
      {
