...
namespace FreeLibSet.Collections
{
  ...
  /// В отличие от обычной коллекции <see cref="Dictionary{String, TValue}"/>, может быть не чувствительна к регистру ключа.
  /// </summary>
  ...
  public class TypedStringDictionary<TValue> : DictionaryWithReadOnly<string, TValue>, INamedValuesAccess
  {
    ...
    /// <summary>
    /// Создает коллекцию и заполняет ее значениями
    /// </summary>
  ...
  /// В отличие от обычной коллекции <see cref="Dictionary{String, TValue}"/>, ключ может быть не чувствителен к регистру ключа.
  ...
  /// Не содержит исходного массива.
  /// Этот класс не является сериализуемым, т.к. легко может быть воссоздан.
  /// Идентичен <see cref="ArrayIndexer{Char}"/> за исключением конструкторов из строки.
  ...
  public sealed class CharArrayIndexer : IComparer<char>
  {
    ...
    /// <summary>
    /// Создает индексатор для массива.
    /// В отличие от <see cref="ArrayIndexer{Char}"/>, допускается наличие в <paramref name="source"/> повторяющихся символов, которые отбрасываются.
    ...
    private readonly Dictionary<char, int> _Dict;
    ...
    /// В отличие от <see cref="Array.IndexOf{Char}(Char[], Char)"/>, выполняется быстро
    /// </summary>
    ...
    /// <summary>
    /// Возвращает true, если в списке содержатся все элементы, то есть если <see cref="Contains(char)"/> возвращает true для каждого элемента.
    ...
    /// <summary>
    /// Возвращает true, если в списке есть хотя бы один элемент, то есть если <see cref="Contains(char)"/> возвращает true для какого-либо элемента.
    ...
    /// отсортировать их в соответствии с порядком элементов в текущем объекте <see cref="CharArrayIndexer"/>.
    ...
    /// начале или в конце списка, в зависимости от свойства <see cref="UnknownItemPosition"/>.
  ...
  public class Int32NamedDictionary : TypedStringDictionary<int>
  {
    ...
    /// Текущая коллекция должна быть доступна для записи (<see cref="DictionaryWithReadOnly{String, Int32}.IsReadOnly"/>=false).
    /// </summary>
    ...
    /// нестатический метод <see cref="Add(IDictionary{String, Int32})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Int32}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    ...
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Substract(IDictionary{String, Int32})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Int32}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
  ...
  public class Int64NamedDictionary : TypedStringDictionary<long>
  {
    ...
    /// <summary>
    /// Добавить к текущей коллекции значения из другой коллекции.
    /// Текущая коллекция должна быть доступна для записи (<see cref="DictionaryWithReadOnly{String, Int64}.IsReadOnly"/>=false).
    /// </summary>
    /// <param name="source">Словарь "Код-Значения", откуда берутся добавляемые значения.
    /// Если null, то никаких действий не выполняется</param>
    ...
    /// Сложение двух коллекций.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Add(IDictionary{String, Int64})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Int64}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
    /// <returns>Новая коллекция</returns>
    ...
    /// Вычитание одной коллекции из другой.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Substract(IDictionary{String, Int64})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Int64}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
  ...
  public class SingleNamedDictionary : TypedStringDictionary<float>
  {
    ...
    /// <summary>
    /// Добавить к текущей коллекции значения из другой коллекции.
    /// Текущая коллекция должна быть доступна для записи (<see cref="DictionaryWithReadOnly{String, Single}.IsReadOnly"/>=false).
    /// </summary>
    /// <param name="source">Словарь "Код-Значения", откуда берутся добавляемые значения.
    /// Если null, то никаких действий не выполняется</param>
    ...
    /// Сложение двух коллекций.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Add(IDictionary{String, Single})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Single}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
    /// <returns>Новая коллекция</returns>
    ...
    /// Вычитание одной коллекции из другой.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Substract(IDictionary{String, Single})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Single}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
    ...
    /// Используйте метод <see cref="Round()"/> после выполнения деления.
    /// Текущая коллекция должна быть доступна для записи (<see cref="DictionaryWithReadOnly{String, Single}.IsReadOnly"/>=false).
    /// </summary>
    /// <param name="d">Делитель. Не может быть равен 0</param>
  ...
  public class DoubleNamedDictionary : TypedStringDictionary<double>
  {
    ...
    /// <summary>
    /// Добавить к текущей коллекции значения из другой коллекции.
    /// Текущая коллекция должна быть доступна для записи (<see cref="DictionaryWithReadOnly{String, Double}.IsReadOnly"/>=false).
    /// </summary>
    /// <param name="source">Словарь "Код-Значения", откуда берутся добавляемые значения.
    /// Если null, то никаких действий не выполняется</param>
    ...
    /// Сложение двух коллекций.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Add(IDictionary{String, Double})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Double}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
    /// <returns>Новая коллекция</returns>
    ...
    /// Вычитание одной коллекции из другой.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Substract(IDictionary{String, Double})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Double}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
  ...
  public class DecimalNamedDictionary : TypedStringDictionary<decimal>
  {
    ...
    /// <summary>
    /// Добавить к текущей коллекции значения из другой коллекции.
    /// Текущая коллекция должна быть доступна для записи (<see cref="DictionaryWithReadOnly{String, Decimal}.IsReadOnly"/>=false).
    /// </summary>
    /// <param name="source">Словарь "Код-Значения", откуда берутся добавляемые значения.
    /// Если null, то никаких действий не выполняется</param>
    ...
    /// Сложение двух коллекций.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Add(IDictionary{String, Decimal})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Decimal}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
    /// <returns>Новая коллекция</returns>
    ...
    /// Вычитание одной коллекции из другой.
    /// При выполнении оператора всегда создается новая коллекция, поэтому обычно экономичнее использовать
    /// нестатический метод <see cref="Substract(IDictionary{String, Decimal})"/>, который мофицирует существующую коллекцию.
    /// Свойство <see cref="TypedStringDictionary{Decimal}.IgnoreCase"/> новой коллекции устанавливается в соответствии со значением свойства первой коллекции.
    /// </summary>
    /// <param name="a">Первая исходная коллекция</param>
    /// <param name="b">Вторая исходная коллекция</param>
