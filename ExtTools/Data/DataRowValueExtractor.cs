...
namespace FreeLibSet.Data
{
  ...
  /// <summary>
  /// Извлечение значения поля с заданным именем из строк <see cref="DataRow"/>.
  ...
  /// При извлечении очередного значения проверяется, относится ли строка <see cref="DataRow"/> к той же <see cref="DataTable"/>, что
  /// и при предыдущем вызове. Если нет, то определяется индекс столбца <see cref="DataColumn"/> в таблице. 
  /// При последующих вызовах используется доступ по индексу поля, а не по имени, что увеличивает скорость
  /// извлечения значения.
  /// Не предназначено для работы с удаленными строками с <see cref="DataRow.RowState"/>=<see cref="DataRowState.Deleted"/> или <see cref="DataRowState.Detached"/>.
  ...
  /// Как правило, структура используется как локальная переменная в пределах одного метода.
  /// 
  /// <see cref="DataRowNullableDateTimeExtractor"/> возвращает значение null, если поле содержит пустое значение,
  /// а <see cref="DataRowDateTimeExtractor"/> - <see cref="DateTime.MinValue"/>.
  /// </summary>
