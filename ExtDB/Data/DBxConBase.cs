...
namespace FreeLibSet.Data
{
  ...
  public abstract class DBxConBase : DisposableObject, IDBxCon
  {
    ...
    private readonly DBxSqlBuffer _Buffer;
    ...
    /// <summary>
    /// Создает объект класса, производного от <see cref="DBxDataWriter"/>.
    /// Непереопределенный метод возвращает "медленный" объект <see cref="DBxDefaultDataWriter"/>.
    /// </summary>
    /// <param name="writerInfo">Параметры обработки</param>
    /// <returns>Объект для записи</returns>
    ...
    /// Непереопределенный метод вызывает <see cref="AddRecord(string, DBxColumns, object[])"/> для каждой строки таблицы.
    /// </summary>
    /// <param name="tableName">Имя таблицы, в которую добавляются записи</param>
    /// <param name="table">Таблица исходных данных</param>
    ...
    /// <summary>
    /// Реализация группового добавления записей с помощью INSERT INTO с заданием VALUES() для множества строк.
    /// Метод может вызываться из переопределенного <see cref="DoAddRecords(string, DbDataReader)"/>, если база данных поддерживает такие запросы.
    /// </summary>
    /// <param name="tableName">Имя таблицы, в которую добавляются записи</param>
    /// <param name="source">Открытая на чтение таблица данных, возможно, в другой базе данных</param>
    ...
    /// <summary>
    /// Позиции столбцов в таблице <see cref="DataTable"/>, входящие в состав первичного ключа, и позиции прочих столбцов.
    /// Предполагается, что таблица содержит все столбцы, входящие в первичный ключ таблицы базы данных.
    /// Первичный ключ <see cref="DataTable.PrimaryKey"/> никак не учитывается.
    /// </summary>
    protected sealed class DataTableColumnPositions
    {
      ...
      /// Совпадает с <see cref="DBxTableStruct.PrimaryKey"/>.
      /// </summary>
      public readonly DBxColumns PKColumnNames;

      /// <summary>
      /// Позиции столбцов в <see cref="DataTable"/>, соответствующие <see cref="PKColumnNames"/>
      /// </summary>
      ...
      /// <summary>
      /// Позиции столбцов в <see cref="DataTable"/>, соответствующие <see cref="OtherColumnNames"/>
      /// </summary>
      ...
      /// <summary>
      /// Создает фильтр <see cref="ValueFilter"/> или <see cref="AndFilter"/> для полей первичного ключа таблицы
      /// </summary>
    ...
    /// <summary>
    /// Реализация группового обновления записей из таблицы <see cref="DataTable"/>.
    /// Метод вызывается из транзакции.
    /// Непереопределенный метод вызывает <see cref="SetValues(string, DBxFilter, DBxColumns, object[])"/>() для каждой строки таблицы <paramref name="table"/>.
    /// </summary>
    /// <param name="tableName">Имя таблицы, в которую добавляются записи</param>
    /// <param name="table">Таблица исходных данных</param>
    ...
    /// Этот метод используется только в <see cref="FindOrAddRecord(string, DBxColumns, object[])"/> и аналогичных классах обновления данных, которые
    ...
    /// В блоке finally должен быть парный вызов <see cref="EndUpdate(string)"/>.
    ...
    /// <summary>
    /// Парный вызов для <see cref="BeginUpdate(string)"/>
    /// </summary>
    /// <param name="tableName">Имя таблицы</param>
    ...
    /// Непереопределенный метод вызывает сначала <see cref="FindRecord(string, DBxColumns, object[])"/>, а затем - <see cref="AddRecord(string, DBxColumns, object[])"/>.
    /// На момент вызова, выполнен вызов <see cref="BeginUpdate(string)"/>.
    /// </summary>
    /// <param name="tableName">Имя таблицы</param>
    ...
    /// Затем для недостающих строк выполняется групповое добавление записей с помощью <see cref="AddRecords(DataTable)"/>.
    /// </summary>
    /// <param name="tableName">Имя таблицы</param>
    /// <param name="table">Строки для поиска. Этот объект не должен меняться</param>
    /// <param name="ids">Сюда должны записываться идентификаторы строк</param>
    ...
    /// Усливие <paramref name="where"/> должно быть задано. Для удаления всех строк таблицы используйте метод <see cref="DeleteAll(string)"/>.
    /// Ссылочные поля (с точками) не поддерживаются.
    /// </summary>
    /// <param name="tableName">Имя таблицы</param>
    ...
    /// </summary>
    /// <param name="sender">Объект <see cref="DataAdapter"/></param>
    /// <param name="args">Аргументы события <see cref="DataAdapter.FillError"/></param>
    ...
    /// <summary>
    /// Возвращает true, если среди переданного списка столбцов <paramref name="columnNames"/> есть ключевое поле из <see cref="DBxTableStruct.PrimaryKey"/>.
    /// </summary>
    /// <param name="tableName">Имя таблицы</param>
    /// <param name="columnNames">Имена полей</param>
    ...
    /// <summary>
    /// Возвращает true, если среди переданного списка столбцов <paramref name="columns"/> есть ключевое поле из <see cref="DBxTableStruct.PrimaryKey"/>.
    /// </summary>
    /// <param name="tableName">Имя таблицы</param>
    ...
    /// Это свойство не дублируется в основном соединении <see cref="DBxCon"/>. Следовательно, проверка может быть отключена
    /// только на стороне сервера (безопасность).
    /// Свойство дублирует <see cref="DBxNameValidator.NameCheckingEnabled"/>.
    /// </summary>
    ...
    /// <summary>
    /// Парный метод для <see cref="SuspendTrace()"/>
    /// </summary>
    ...
    {
      DBxConQueryInfo ti = new DBxConQueryInfo(cmdText, TraceEnabled && _TraceSuspendCount == 0, _CurrentQuery);
      _CurrentQuery = ti;
