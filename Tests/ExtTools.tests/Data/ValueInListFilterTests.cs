...
{
  [TestFixture]
  public class ValueInListFilterTests
  {
    ...
    public void Constructor_expr_3args()
    {
      ...
      ValueInListFilter sut = new ValueInListFilter(expr, values, DBxColumnType.Double);
      Assert.AreSame(expr, sut.Expression, "Expression");
      Assert.AreSame(values, sut.Values, "Values");
      Assert.AreEqual(DBxColumnType.Double, sut.ColumnType, "ColumnType");
    ...
    public void Constructor_expr_2args()
    {
      ...
      ValueInListFilter sut = new ValueInListFilter(expr, values);
      Assert.AreSame(expr, sut.Expression, "Expression");
      Assert.AreSame(values, sut.Values, "Values");
    ...
    {
      int[] values = new int[] { 10, 20, 0 };
      ValueInListFilter sut = new ValueInListFilter("F1", values, DBxColumnType.Decimal);
      ...
      Assert.AreSame(values, sut.Values, "Values");
      Assert.AreEqual(DBxColumnType.Decimal, sut.ColumnType, "ColumnType");
      Assert.AreEqual(DBxFilterDegeneration.None, sut.Degeneration, "Degeneration");
    }
    ...
    public void Constructor_colName_2args()
    {
      ...
      ValueInListFilter sut = new ValueInListFilter("F1", values);
      Assert.IsInstanceOf<DBxColumn>(sut.Expression, "Expression type");
      Assert.AreEqual("F1", ((DBxColumn)(sut.Expression)).ColumnName, "ColumnName");
      Assert.AreSame(values, sut.Values, "Values");
      Assert.AreEqual(DBxFilterDegeneration.None, sut.Degeneration, "Degeneration");
    ...
    public void TestFilter_int(string sListValues, object testValue, bool wantedRes)
    {
      ...
      ValueInListFilter sut = new ValueInListFilter("F1", listValues);
    ...
    public void TestFilter_diffTypes_String_Guid(string sListValues, string testValue, bool wantedRes)
    {
      ...
      ValueInListFilter sut1 = new ValueInListFilter("F1", listValues1);
      ValueInListFilter sut2 = new ValueInListFilter("F2", listValues2);
    ...
    {
      ValueInListFilter sut = new ValueInListFilter("F1", new int[] { 1, 3, 5 }, DBxColumnType.Decimal);
      byte[] b = SerializationTools.SerializeBinary(sut);
      ValueInListFilter res = (ValueInListFilter)(SerializationTools.DeserializeBinary(b));
