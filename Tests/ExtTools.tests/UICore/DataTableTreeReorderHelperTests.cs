...
namespace ExtTools_tests.UICore
{
  ...
  public class DataTableTreeReorderHelperTests
  {
    ...
    // Ќе провер€ем возможность сдвига дл€ разноуровневых узлов, т.к. может в будущей версии будет разрешено
    ...
    {
      // ѕеремещение дл€ нескольких строк должно работать независимо от пор€дка задани€ выбранных строк
      for (int i = 1; i <= 2; i++)
      {
        DataTableTreeReorderHelper sut = CreateTestObject();
    ...
    [TestCase("AAA", "", false, "AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH")]
    [TestCase("DDD", "DDD", true, "AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH")] // хот€ пор€док и не помен€лс€
    ...
    public void InitRows(string sRowValues, string sZeroRowValues, bool wantedRes, string sWantedValues)
    {
      ...
      // Ќе провер€ем otherRowChanged, т.к. непредсказуемо
    ...
    public void Reorder_Simple()
    {
      ...
      Assert.AreEqual("AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH", GetOrderedText(sut.Model.Table), "Rows #1"); // ничего не помен€лось

      bool res2 = sut.Reorder();
      Assert.IsFalse(res2, "Result #2");
      Assert.AreEqual("AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH", GetOrderedText(sut.Model.Table), "Rows #2"); // ничего не помен€лось
    }

    [TestCase("AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH", "AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH")] // не мен€ем
    [TestCase("FFF,BBB,AAA,DDD,CCC,EEE,GGG,HHH", "AAA,BBB,CCC,DDD,EEE,FFF,GGG,HHH")] // относительно групп пор€док сохранилс€
    [TestCase("DDD,BBB,CCC,AAA,EEE,FFF,GGG,HHH", "DDD,EEE,FFF,GGG,HHH,AAA,BBB,CCC")] // перестановка "AAA"-"DDD"
    [TestCase("HHH,GGG,FFF,EEE,DDD,CCC,BBB,AAA", "DDD,HHH,GGG,EEE,FFF,AAA,CCC,BBB")] // обратный пор€док
    public void Reorder_ByRows(string sRowValues, string sWantedValues)
    {
      ...
      // ¬ текущей реализации и при повторном вызове может вернуть true.
      //Assert.IsFalse(res2, "Result #2");
      Assert.AreEqual(sWantedValues, GetOrderedText(sut.Model.Table), "Rows #2"); // ничего не помен€лось
    }

    #endregion

    #region “естова€ таблица
    ...
    private static DataTable CreateTestTable()
    {
      ...
      // Ќельз€ использовать значени€ не по пор€дку дл€ пол€ "Order"
      // ћодель работать будет, но тестовый метод GetOrderedText() будет возвращать неправильные результаты
    ...
    private static DataRow[] GetTableRows(DataTable tbl, string sRowValues)
    {
      ...
      for (int i = 0; i < aValues.Length; i++)
      {
        ...
        if (p < 0)
          throw new BugException("«начени€ \"" + aValues[i] + "\" нет в таблице");
        rows[i] = tbl.Rows[p];
      }
      return rows;
    }
