...
namespace ExtTools_tests.UICore
{
  ...
  public class DataTableReorderHelperTests
  {
    ...
    public void InitRows_None()
    {
      ...
      Assert.IsFalse(otherRowChanged, "otherRowChanged");
      Assert.AreEqual("AAA,DDD,CCC,BBB", GetOrderedText(tbl), "Rows"); // ничего не помен€лось
    }
    ...
    public void Reorder_Simple()
    {
      ...
      Assert.AreEqual("AAA,DDD,CCC,BBB", GetOrderedText(tbl), "Rows #1"); // ничего не помен€лось

      bool res2 = sut.Reorder();
      Assert.IsFalse(res2, "Result #2");
      Assert.AreEqual("AAA,DDD,CCC,BBB", GetOrderedText(tbl), "Rows #2"); // ничего не помен€лось
    }
    ...
    public void Reorder_ByRows()
    {
      ...
      Assert.IsFalse(res2, "Result #2");
      Assert.AreEqual("DDD,CCC,BBB,AAA", GetOrderedText(tbl), "Rows #2"); // ничего не помен€лось
    }
    ...
    private static DataTable CreateTestTable()
    {
      ...
      // “екущий пор€док сортировки: "AAA,DDD,CCC,BBB" (10,20,30,40)
      return tbl;
    }
