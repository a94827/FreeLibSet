...
{
  /// <summary>
  /// “естирование объекта DBxMultiSubDocs в режиме, используемом дл€ редактировани€ поддокументов из таблицы DBxSubDocGridView в редакторе документа.
  ///  онструктор DBxMultiSubDocsTests создает отдельный набор на врем€ редактировани€ поддокумента(ов), который затем сливаетс€ с основным набором.
  /// </summary>
  [TestFixture]
  public class DBxMultiSubDocsTests : DBxDocSetTestsBase
  {
    ...
    public void Constructor_DataRows([Values(false, true)] bool useDeleted, [Values(false, true)] bool useVersions, [Values(false, true)] bool useTime)
    {
      ...
      doc.SubDocs["SD11"][0].Values["F111"].SetString("DDD"); // мен€ем значение
      ...
      Assert.AreEqual("DDD", sut[0].Values["F111"].AsString, "Value[0]"); // «начение изменено
    ...
    public void Scenario_Insert_Insert([Values(false, true)] bool useDeleted, [Values(false, true)] bool useVersions, [Values(false, true)] bool useTime)
    {
      ...
      DBxDocSet ds = new DBxDocSet(info.Provider);
      DBxSingleDoc doc = ds["D1"].Insert();
      doc.Values["F101"].SetBoolean(false); // об€зательное поле
    ...
    public void Scenario_Edit_Insert([Values(false, true)] bool useDeleted, [Values(false, true)] bool useVersions, [Values(false, true)] bool useTime)
    {
      ...
      // ѕосле добавлени€, поддокументы могли оказатьс€ в другом пор€дке.
      // ≈сли сортировка по умолчанию выполн€етс€ по идентификатору поддокумента, то он будет первым по
      // счету, но на это не стоит полагатьс€
      ...
      // ѕосле повторного открыти€ документа, поддокументы будут идти в предсказуемом пор€дке
